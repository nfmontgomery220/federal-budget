...
export async function GET(request: Request) {
  ...
  try {
    console.log(`[v0] Looking up Senators for ZIP: ${zip}`)
    ...
    if (!state) {
      return NextResponse.json(
        { error: "Unable to determine state from ZIP code", representatives: [] },
        { status: 404 },
      )
    }
    ...
    console.log(`[v0] ✓ Found ${senators.length} senators for ${state}`)
    ...
    console.error("[v0] Error fetching senators:", error)
    return NextResponse.json({ error: "Lookup failed. Please try again.", representatives: [] }, { status: 500 })
  }
}

// Simple ZIP prefix to state mapping (covers 99% of ZIP codes)
function getStateFromZip(zip: string): string | null {
  const prefix = Number.parseInt(zip.substring(0, 3))

  const zipMap: Record<number, string> = {
    // Alabama
    350: "AL",
    351: "AL",
    352: "AL",
